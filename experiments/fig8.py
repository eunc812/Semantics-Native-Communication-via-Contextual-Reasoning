"""
Fig 8 reproduction (paper Section IV): Robustness to Asynchronous Contextual Reasoning in System 2 SNC.

- x-axis: Perturbation ε (0 ~ 0.05), y-axis: Reliability γ (0 ~ 1).
- Two curves:
    (1) A2C and C2A without Quantization (blue, dash-dot, triangle)
    (2) A2C and C2A with Quantization    (yellow/gold, solid, circle)
- Asynchronous model: speaker and listener derive A2C/C2A from different P_Xc_given_A and
  initialize self-SNC independently. Paper: "speaker and listener having different A2C and C2A
  when initializing their self-SNC procedure."
- Perturbation: add uniform[-ε,+ε] to each component of P_Xc_given_A then [0,1] clip.
- Quantization: round P_Xc_given_A to one decimal (np.round(P, 1)) then derive A2C/C2A.
  Under Beta(0.1,0.1) most values are near 0 or 1, so ε ≤ 0.05 perturbation stays in same
  quantization bin, ensuring robustness.
- Paper setup: |A|=|C|=100, λ=0.5, α=1.1, β=1.5, K=3 communication rounds.
"""
...
def _generate_beta_world(
    ...
    """Paper hyperparameter pair (0.1, 0.1) → Beta(0.1, 0.1) world."""
...
def _async_algorithm1_infer(
    ...
    """
    Asynchronous Algorithm 1: speaker/listener derive A2C/C2A from different P;
    after K communication rounds return listener inferred action.
    """
...
def _run_trial_pair(
    ...
    """
    Single trial: return (no-quant, quant) gamma pair with same perturbation/action sample.
    """
...
def compute_fig8_data(
    ...
    """
    Fig 8 data computation.
    Returns (gamma_no_quant, gamma_with_quant), each shape (len(epsilons),).
    """
    P_true, pA, pC = world
    rng = np.random.default_rng(seed)
    n_eps = len(epsilons)
...
def plot_fig8(
    ...
    try:
        ...
        raise ImportError("matplotlib is required to plot Fig 8: pip install matplotlib")
    ...
    print("Computing Fig 8 data (K={}, n_trials={}, n_sample={}, max_iter={})...".format(
    ...
    # fig7 palette: gold=#E5A800, deep blue-violet=#2F13B8
    color_q = "#E5A800"
    color_nq = "#2F13B8"
