"""
Fig 4 재현 (논문 Section IV).

- Fig 4(a): G 수렴 곡선. |A|=|C|=100, λ=0.5, α=β=1.1/1.5/2.
- Fig 4(b): stationary rA2C 히트맵. |E|=|A|=10, |C|=20, β=1.5, α=1.1/1.5/2.
  "empirical distribution of chosen meaningful concept", 0=검정/1=연한 베이지.
system2 기존 함수만 사용.
"""
...
# Fig 4(b) 재현용 고정 seed (raw 기준 alpha=2.0 집중이 강한 seed)
...
def _build_fig4b_world(
    ...
    """4(b)용 S0, L0 생성. 반환 (S0, L0, pC_A, pA)."""
...
def _entity_row_order_for_fig4b(rA2C_alpha2: np.ndarray) -> np.ndarray:
    """
    원본 확률값은 유지한 채 경향성 가독성만 높이기 위한 행 정렬.
    α=2.0에서 지배 컨셉(argmax) 오름차순, 동일 그룹 내 row-max 내림차순.
    """
...
def verify_fig4b_concentration(
    ...
    """
    α↑에 따라 행이 한 컨셉에 몰리는지 검증.
    - min_last_mean_row_max: α 최대일 때 mean(row_max) 하한.
    - min_rows_confirmed_at_last: α=2.0일 때 row_max >= confirmed_thresh 인 행 수 하한 (entity-concept 확정).
    반환: (통과 여부, 요약 문자열).
    """
...
def plot_fig4(
    ...
    """
    논문 Fig 4 한 창: (a) 왼쪽 G 수렴 곡선, (b) 오른쪽 rA2C 히트맵.
    제목 "Fig. 4.", 각 블록 아래 (a), (b).
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.colors import LinearSegmentedColormap
    except ImportError:
        raise ImportError("Fig 4를 그리려면 matplotlib이 필요합니다: pip install matplotlib")
    ...
    # (b) 단일 고정 실험 world (후처리 없이 raw rA2C 사용)
...
def plot_fig4a(
    ...
    """
    Fig 4(a): G 수렴 곡선. α=β=1.1, 1.5, 2 각각 서브플롯.
    x축: Iteration steps (1 ~ max_iter), y축: Objective G.
    """
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        raise ImportError("Fig 4(a)를 그리려면 matplotlib이 필요합니다: pip install matplotlib")
    ...
    for idx, (alpha, beta) in enumerate(alphas_betas):
        ...
        # step 0 = 초기 G (논문 형태). S0,L0,M0를 joint로 정규화 후 G
...
def plot_fig4b(
    ...
    """
    Fig 4(b): stationary rA2C 히트맵 (empirical distribution of chosen meaningful concept).
    α=1.1, 1.5, 2.0 (β=1.5 고정), |E|=10, |C|=20.
    색: 0=검정, 1=연한 베이지. 컬러바는 그림과 겹치지 않게 오른쪽에 배치.
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.colors import LinearSegmentedColormap
    except ImportError:
        raise ImportError("Fig 4(b)를 그리려면 matplotlib이 필요합니다: pip install matplotlib")
    ...
    # 후처리 없이, 경향성이 잘 보이는 고정 seed 단일 실험 수행.
    fixed_seed = FIG4_FIXED_SEED
    max_iter_4b = FIG4B_MAX_ITER
    tol_4b = FIG4B_TOL
    dirichlet_4b = FIG4B_DIRICHLET_ALPHA
